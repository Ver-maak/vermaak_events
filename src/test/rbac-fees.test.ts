...
describe("RBAC — only super admins can modify pricing", () => {
  it("anon cannot update exchange_rates", async () => {
    const { error } = await anon.from("exchange_rates").update({ rate_to_ugx: 1 }).eq("currency", "USD");
    expect(error).toBeTruthy();
  });

  it("anon cannot insert a fee_tier", async () => {
  ...
  it("anon cannot insert a fee_tier_version", async () => {
  ...
  it("anon cannot update currency_rounding", async () => {
    const { error } = await anon.from("currency_rounding").update({ decimals: 9 }).eq("currency", "USD");
    expect(error).toBeTruthy();
  });
